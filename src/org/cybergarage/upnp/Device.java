...
public class Device implements HTTPRequestListener, SearchListener {
	...
	/**
	 * Add a Device to device without checking for duplicate or syntax error. This method set or reset the root node of the Device and itself<br>
	 * <br>
...
//        devNT = "urn:schemas-upnp-org:device:TV:1";
//        devUSN = getNotifyDeviceTypeUSN();
//        ssdpReq.setNT(devNT);
//        ssdpReq.setUSN(devUSN);
//        ssdpSock.post(ssdpReq);
	...
	/*
	 * public void output(PrintWriter ps) { ps.println("deviceType = " + getDeviceType()); ps.println("freindlyName = " + getFriendlyName()); ps.println("presentationURL = " + getPresentationURL()); DeviceList devList = getDeviceList(); ps.println("devList = " + devList.size()); ServiceList
	 * serviceList = getServiceList(); ps.println("serviceList = " + serviceList.size()); IconList iconList = getIconList(); ps.println("iconList = " + iconList.size()); } public void print() { PrintWriter pr = new PrintWriter(System.out); output(pr); pr.flush(); }
	 */
