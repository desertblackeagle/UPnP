...
public class HTTPMUSocket
{
    ...
    /**
     * @param addr
     *            {@link String} rappresenting the multicast hostname to join
     *            into.
     * @param port
     *            int rappresenting the port to be use poth as source and
     *            destination
     * @param bindAddr
     *            {@link InetAddress} which identify the hostname of the
    ...
    public SSDPPacket receive() throws IOException
    {
        ...
        if (ssdpMultiSock != null)
            ssdpMultiSock.receive(recvPacket.getDatagramPacket()); // throws
                                                                   // IOException
        else
